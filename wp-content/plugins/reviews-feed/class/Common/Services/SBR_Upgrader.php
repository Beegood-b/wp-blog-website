...
class SBR_Upgrader  extends ServiceProvider {
	...
	const SLUG = 'reviews-feed-pro/sb-reviews-pro.php';

	const REDIRECT = 'sbr-settings';

	const INSTALL_INSTRUCTIONS = 'https://smashballoon.com/doc/setting-up-the-reviews-feed-pro-wordpress-plugin/?reviews&utm_campaign=reviews-free&utm_source=settings&utm_medium=freetopro&utm_content=Upgrade Manually';
	...
	public function hooks() {
		add_action( 'wp_ajax_nopriv_sbr_run_one_click_upgrade', array( 'SmashBalloon\Reviews\Common\Services\SBR_Upgrader', 'install_upgrade' ) );
		add_action( 'wp_ajax_sbr_maybe_upgrade_redirect', array( 'SmashBalloon\Reviews\Common\Services\SBR_Upgrader', 'maybe_upgrade_redirect' ) );
	}
	...
	public static function maybe_upgrade_redirect() {
		...
		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'You are not allowed to install plugins.', 'reviews-feed' ) ) );
		}
		...
		if ( empty( $license ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'You are not licensed.', 'reviews-feed' ) ) );
		}
		...
		if ( ! is_wp_error( $response ) ) {
			...
			update_option( 'sbr_license_key', $license );
			update_option( 'sbr_license_data', $license_data );
			update_option( 'sbr_license_status', $license_data['license'] );

			// Redirect.
			$oth = hash( 'sha512', wp_rand() );
			$hashed_oth = hash_hmac( 'sha512', $oth, wp_salt() );
			update_option( 'sbr_one_click_upgrade', $oth );
			$version      = '1.0';
		...
		wp_send_json_error( array( 'message' => esc_html__( 'Could not connect.', 'reviews-feed' ) ) );
	}
